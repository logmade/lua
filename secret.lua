...
do 
	...
	do 
		...
		do 
			...
			utility.drawings["Sub"] = utility:drawObject("Text", {Outline = true, Text = "og", Color = Color3.new(1, 1, 1), Size = 13})
			...
			do 
				Messages = {
					...
					["og.vip"] = {
						...
						"OG OG OGNNN RAHHHHH",
						"Slammed by OG.VIP",
						"YOU GOT SLAMMED BY OG",
						"SEEMS LIKE YOU NEED ogvip GG/RESALE",
						"GG/RESALE GG/RESALE GG/RESALE",
						"GG/RESALE <-- THIS WILL LET YOU COPE WITH YOUR ISSUES",
						"WHAT YOU CANT BEAT OG.VIP?",
						"PRO TIP: GET OG",
					},
				}
				utx = {
...
local Window = Library:Window({Size = UDim2.new(0,580,0,600)}) do 
	...
	local misc = Window:Page({Name = "Misc", Weapons = true}) do 
		...
		local chatSpam = Movement:Section({Name = "Chat Spam", Size = 330, Side = "Left"}) do
			...
			chatSpam:Dropdown({Name = "Type", Flag = "Chat Spam Type", Options = {"og.vip", "Scottish"}, Default = "og.vip"})
	...
	local Config = Window:Page({Name = "Settings"}) do
		...
		--
		local CurrentList = {};
		if not isfolder("og.vip") then 
			makefolder("og.vip")
		end 
		if not isfolder("og.vip/Configs") then 
			makefolder("og.vip/Configs")
		end 
		local function UpdateConfigList()
			local List = {};
			for idx, file in ipairs(listfiles("og.vip/Configs")) do
				local FileName = file:gsub("og.vip/Configs\\", ""):gsub(".cfg", "")
		...
		Cfgs:Button({Name = "Create", Callback = function()
			...
			if ConfigName == "" or isfile("og.vip/Configs/" .. ConfigName .. ".cfg") then
				return;
			end;
			writefile("og.vip/Configs/" .. ConfigName .. ".cfg", Library:GetConfig());
			UpdateConfigList();
		end})
		Cfgs:Button({Name = "Save", Callback = function()
			local SelectedConfig = flags.SettingConfigurationList;
			if SelectedConfig then
				writefile("og.vip/Configs/" .. SelectedConfig .. ".cfg", Library:GetConfig())
			end;
		end})
		Cfgs:Button({Name = "Load", Callback = function()
			local SelectedConfig = flags.SettingConfigurationList;
			if SelectedConfig then
				Library:LoadConfig(readfile("og.vip/Configs/" .. SelectedConfig .. ".cfg"))
			end
		end})
		Cfgs:Button({Name = "Delete", Callback = function()
			local SelectedConfig = flags.SettingConfigurationList;
			if SelectedConfig then
				delfile("og.vip/Configs/" .. SelectedConfig .. ".cfg")
			end
			UpdateConfigList();
		end})
...
Library:Notification("Loaded og.vip in " .. Floor((tick() - loadingTime) * 1000)  .."ms", 3, Library.Accent, flags["Notification Position"])
...
credit @logmade if you want to skid my skidding >.<
